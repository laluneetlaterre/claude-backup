---
name: reference_miey_public_profiles
description: Miey の公開プロフィール各種（X / note / Cafetalk / LinkedIn）とフォロワー数
metadata: 
  node_type: memory
  type: reference
...
- **X (Twitter)**: `https://x.com/diaita1` — フォロワー **2,652**（2026-06-24 時点）。reach が一番大きい主戦場。**X Premium 契約済み（認証バッジあり）。無課金ではない**ので、リーチ評価時に「無課金だから」と前提を間違えないこと。
- **note**: `https://note.com/miey_casa` — フォロワー約 **54**。
- **Cafetalk（オンライン講師プロフィール）**: `https://cafetalk.com/tutor/profile/?c=eJwr8vZNzytJNzQL9EyK1A9yKjQINw4ItLUFAGPiB4A.&lang=ja` — 「注目講師ランキング1位」「リピート/口コミランキング」入りの実績あり（四柱推命講座 10,000pt がよく売れた）。
