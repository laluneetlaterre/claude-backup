---
name: project_stock_content_strategy
description: SNS依存脱却→note/YouTubeストック型・1記事戦略＋副産物設計。発信チャネルと商品の売り方の指針（数字は仮説扱い）
metadata: 
  node_type: memory
  type: project
...
2026-06-14、Miey が note/YouTube を軸とする「ストック型発信戦略」に舵を切る方針を固めた。出典は note 系発信者のノウハウを NotebookLM で要約した戦略文書 `03_stock/04_marketing/2026-06-14_note×YouTubeストック型発信戦略_脱SNS依存と1記事戦略.md`。

**核:**
- SNS（X等フロー型）は集客の主役から「脇役（信頼の受け皿・補助流入）」へ降格。やめはしないが期待を下げる。
- note（ストック型・検索流入・資産性）を主軸に。
- 1記事戦略：低単価量産でなく高単価（5,000〜30,000円）の渾身の1記事を育て続ける。
- 副産物（バックエンド）設計：記事売上の数倍。アフィリ・業者提携・サブスク・自社サービス（Cafetalk 等）。
- note→YouTube 横展開：反応の良い記事を動画化。「note」キーワード自体が YouTube で強い。
- 属人型（一次情報・体験談・過程の共有）。AI 量産型は NG。

**商品への適用:** Obsidian構築商品＝本編を note 高単価1記事に、副産物＝Cafetalk個別サポート・拡張編（Skills）・アフィリ。[[project_business_value_ladder]] と整合。
